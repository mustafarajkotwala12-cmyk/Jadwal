...
public struct ClassCardView: View {
    ...
    private var frontCard: some View {
        VStack(alignment: .leading, spacing: 7) {
            // Top row: Period Badge + Status pill
            HStack(alignment: .center, spacing: 6) {
                Text(period.periodName)
                    ...
                    .background(
                        Capsule()
                            .fill(status == .inProgress ? FatimidPalette.emeraldSoft : Color.accentColor.opacity(0.12))
                    )
                    .foregroundStyle(status == .inProgress ? FatimidPalette.emerald : Color.accentColor)
            ...
            // Subject name: Large, readable, rendered with KanzalLulu for Arabic
            Text(period.subject)
                .font(period.subject.containsArabic ? .kanzalLulu(size: 26) : .system(size: 20, weight: .bold, design: .rounded))
            ...
            if let change = period.changeRecord, !change.isAcknowledged {
                HStack(spacing: 3) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .font(.system(size: 9))
                    Text(change.changeType == .subjectChanged ? "Changed" : "Updated")
            ...
            // Bottom row: Tasks count + Flip affordance
            HStack {
                Button(action: toggleFlip) {
                    HStack(spacing: 4) {
                        Image(systemName: "checklist")
                            .font(.system(size: 10, weight: .semibold))
                        Text(linkedTasks.isEmpty ? "Tasks" : "\(linkedTasks.count) Task\(linkedTasks.count == 1 ? "" : "s")")
    ...
    private var backCard: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            // Subject indicator
            Text(period.subject)
                .font(period.subject.containsArabic ? .kanzalLulu(size: 13) : .system(size: 11, weight: .semibold))
                .foregroundStyle(.secondary)
                .lineLimit(1)
            ...
            // Task items list
            if linkedTasks.isEmpty && completedLinkedTasks.isEmpty {
                VStack(spacing: 4) {
                    Spacer()
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        ForEach(completedLinkedTasks) { task in
                            taskRow(task)
                        }
                    }
                }
    ...
    private var cardBackground: some View {
        RoundedRectangle(cornerRadius: 16, style: .continuous)
            ...
            .shadow(
                color: status == .inProgress ? FatimidPalette.emerald.opacity(0.15) : Color.black.opacity(0.04),
    ...
    private var borderColor: Color {
        ...
        default:
            return Color.secondary.opacity(1)
        }
    }

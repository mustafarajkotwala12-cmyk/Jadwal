...
import SwiftUI
import JadwalCore
import JadwalPersistence
import JadwalIntegrations
...
public final class AppEnvironment: ObservableObject {
    ...
    public init() {
        ...
        let jadwalDataDir = appSupport.appendingPathComponent("Jadwal", isDirectory: true)
        let legacyDataDir = appSupport.appendingPathComponent("SWIFT", isDirectory: true)
        let storageDir = FileManager.default.fileExists(atPath: jadwalDataDir.path) || !FileManager.default.fileExists(atPath: legacyDataDir.path) ? jadwalDataDir : legacyDataDir

        let timetableRepo = LocalFileTimetableRepository(storageDirectory: storageDir)
        let taskRepo = LocalFileTaskRepository(storageDirectory: storageDir)
        self.changeRepository = LocalFileChangeRepository(storageDirectory: storageDir)

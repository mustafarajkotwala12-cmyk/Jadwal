# Jadwal Feature Changelog & Architecture Inventory

This document tracks all features, capabilities, and system components incorporated into the **Jadwal** native macOS application and the companion **Jamea Helper** pipeline.

---

## System Overview
- **App Name**: Jadwal (Aljamea-tus-Saifiyah Academic Schedule & Workflow Companion)
...
### 2. Timetable Importer & Bridge (`JadwalIntegrations`)
- **`TimetableImporter`**:
...
### 3. Change Management & Task Protection Engine (`JadwalCore`)
...
- **Menu Bar Extra Companion (`MenuBarView`)**:
  ...
  - "Open Jadwal" and "Quit" shortcuts.

### 5. Persistence Layer (`JadwalPersistence`)
- **Actor-Isolated Thread-Safe File Repositories**:
  ...
  - Directory: `~/Library/Application Support/Jadwal/` (macOS standard).
